...
namespace qtuser_3d
{
	...
	qtuser_3d::XRenderPass* RenderPassManager::xrenderPass(const QString& name)
	{
		QMap<QString, qtuser_3d::XRenderPass*>::iterator it = m_xrenderPasses.find(name);
		if (it != m_xrenderPasses.end()) return (*it);

		qtuser_3d::XRenderPass* pass = xcreate(name, m_root);
		if (pass)
		{
			it = m_xrenderPasses.insert(name, pass);
			if (it == m_xrenderPasses.end())
			{//insert error
				delete pass;
				pass = nullptr;
			}
	...
	qtuser_3d::XRenderPass* RenderPassManager::xcreate(const QString& name, Qt3DCore::QNode* parent)
	{
		qtuser_3d::XRenderPass* pass = new qtuser_3d::XRenderPass(parent);
		...
		{
			shaderName = passSegs.at(0);
			pass->addFilterKeyMask(filterName, 0);
		}
		else if (passSegs.size() > 1)
		{
			shaderName = passSegs.at(0);
			for (int i = 1; i < passSegs.size(); ++i)
			{
				pass->addFilterKeyMask(passSegs.at(i), 0);
			}
		}
		pass->setObjectName(shaderName);
